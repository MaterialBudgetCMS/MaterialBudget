...
import MaterialBudget.CMGTools.Das as Das

from MaterialBudget.CMGTools.dataset import Dataset
...
class CheckDatasetExists(Task):
    ...
    Colin: one should rather use the MaterialBudget.CMGTools.dataset module
    """
    def __init__(self, dataset, user, options):
...
class GenerateMask(Task):
    ...
    def run(self, input):
        ...
        if self.options.check and not input['CheckForMask']['MaskPresent']:
            from MaterialBudget.CMGTools.edmIntegrityCheck import IntegrityCheck, PublishToFileSystem
...
class WriteToDatasets(Task):
    ...
    def run(self, input):
        from MaterialBudget.CMGTools.addToDatasets import addToDatasets
