...
void Resolution::Loop()
{
  ...
  for(int IFile = 0; IFile < 1; IFile++)
  {
   ...
   if(IFile == 1) {
    outputFile = new TFile("ResolutionPlots_100GeV_2015.root", "RECREATE"); //100 GeV
   }
   */
   ...
   hMC_pt_cut_max = new TH1D("hMC_pt_cut_max", "CMS work in Progress", 105, 0.0, 105.0);
   //hMC_pt_cut_max->Sumw2();
   hMC_pt_cut_all = new TH1D("hMC_pt_cut_all", "CMS work in Progress", 105, 0.0, 105.0);
   //hMC_pt_cut_all->Sumw2();
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      double momentumInc_pt_max = MC_TrkV_momentumInc_pt->at(0);
      //cout << "Event # " << jentry << endl;
      //cout << "Initial = " << MC_TrkV_momentumInc_pt->at(0) << endl;
      for ( int j = 0; j < numberOfMC_TrkV; j++ )
      {
       hMC_pt_cut_all->Fill(MC_TrkV_momentumInc_pt->at(j));
       if(momentumInc_pt_max < (MC_TrkV_momentumInc_pt->at(j)))
       {
	momentumInc_pt_max = (MC_TrkV_momentumInc_pt->at(j));
       }	  
      }
      hMC_pt_cut_max->Fill(momentumInc_pt_max);
      for ( int j = 0; j < numberOfMC_TrkV; j++ )
      {
	  ...
	  {
	   count1++;
	   if(MC_TrkV_momentumInc_pt->at(j) <= 9.0 || MC_TrkV_momentumInc_pt->at(j) >= 11.0) continue;
	  }
	  else if(IFile == 1)
	  {
	   count2++;
           if(MC_TrkV_momentumInc_pt->at(j) <= 95.0 ) continue;
	  }
	  */
   ...
   //cout << "low " << hMC_deltaR3d << endl;
   //cout << "high " << count2 << endl;
 }
}
