#ifndef CutAnalyzer_cxx
#define CutAnalyzer_cxx
...
CutAnalyzer::CutAnalyzer( const std::string fileList, const Long64_t maxEvents )
{
  ...
  outputFile = new TFile( "prova_Cuts2017.root", "RECREATE" );
...
void CutAnalyzer::beginJob()
{
  ...
  PFDV_vTrack_algo = new std::vector< std::vector< double > >();
  ...
  if (  inputChain->GetBranchStatus( "PFDV_vTrack_algo" ) )
    inputChain->SetBranchAddress( "PFDV_vTrack_algo", &PFDV_vTrack_algo, &b_PFDV_vTrack_algo );
  else std::cout << "PFDV_vTrack_algo" << warningString_Branches.data() << std::endl;
  if (  inputChain->GetBranchStatus( "PFDV_vTrack_chi2" ) )
    inputChain->SetBranchAddress( "PFDV_vTrack_chi2", &PFDV_vTrack_chi2, &b_PFDV_vTrack_chi2 );
  else std::cout << "PFDV_vTrack_chi2" << warningString_Branches.data() << std::endl;
  if (  inputChain->GetBranchStatus( "PFDV_vTrack_normalizedChi2" ) )
    inputChain->SetBranchAddress( "PFDV_vTrack_normalizedChi2", &PFDV_vTrack_normalizedChi2, &b_PFDV_vTrack_normalizedChi2 );
  else std::cout << "PFDV_vTrack_normalizedChi2" << warningString_Branches.data() << std::endl;
  ...
  hMC_TrkV_momentumOut_pt_Barrel = new TH1D( "hMC_TrkV_momentumOut_pt_Barrel", "N.I. in Tracker", 100, 0., 60.0 );
  hMC_TrkV_momentumOut_pt_Barrel->Sumw2();
  hMC_TrkV_momentumInc_pt_Barrel = new TH1D( "hMC_TrkV_momentumInc_pt_Barrel", "N.I. in Tracker", 100, 0., 60.0 );
  ...
  hMC_TrkV_momentum_dtheta_Barrel = new TH1D( "hMC_TrkV_momentum_dtheta_Barrel", "N.I. in Tracker", 100, 0., 0.2 );
  ...
  hMC_TrkV_momentumOut_phi_Barrel = new TH1D( "hMC_TrkV_momentumOut_phi_Barrel", "N.I. in Tracker", 100, -TMath::Pi(), TMath::Pi());
  ...
  hMC_TrkV_momentumOut_pt_Barrel_MaxCut = new TH1D( "hMC_TrkV_momentumOut_pt_Barrel_MaxCut", "N.I. in Tracker", 100, 0., 60. );
  hMC_TrkV_momentumOut_pt_Barrel_MaxCut->Sumw2();
  hMC_TrkV_momentumOut_pt_Forward_MaxCut = new TH1D( "hMC_TrkV_momentumOut_pt_Forward_MaxCut", "N.I. in Tracker", 100, 0., 60. );
  ...
  hMC_TrkV_momentumInc_phi_Barrel = new TH1D( "hMC_TrkV_momentumInc_phi_Barrel", "N.I. in Tracker", 100, -TMath::Pi(), TMath::Pi());
  ...
  hPFDV_vTrack_pt = new TH1D( "hPFDV_vTrack_pt", "CMS work in progress", 100, 0, 60 );
  hPFDV_vTrack_pt->Sumw2();

  hPFDV_vTrack_eta = new TH1D( "hPFDV_vTrack_eta", "CMS work in progress", 100, -2.5, 2.5);
  hPFDV_vTrack_eta->Sumw2();

  hPFDV_vTrack_phi = new TH1D( "hPFDV_vTrack_phi", "CMS work in progress", 100, -TMath::Pi(), TMath::Pi());
  ...
  //RECO Cuts
  hPFDV_momentumInc_pt = new TH1D( "hPFDV_momentumInc_pt", "CMS work in progress", 100, 0., 60.);
  ...
  hPFDV_momentumInc_phi = new TH1D( "hPFDV_momentumInc_phi", "CMS work in progress", 100, -TMath::Pi(), TMath::Pi());
  ...
  hPFDV_vTrack_closestDxyPVIdx_dxy = new TH1D( "hPFDV_vTrack_closestDxyPVIdx_dxy", "CMS work in progress", 100, 0.0, 10.0);
  ...
  hPFDV_vTrack_closestDzPVIdx_dxy = new TH1D( "hPFDV_vTrack_closestDzPVIdx_dxy", "CMS work in progress", 100, 0.0, 10.0);
...
// start RECO part
    ...
    for ( unsigned int i = 0; i < numberOfPFDV; i++ )
    {
      ...
      if(PFDV_numberOfTracks_0p2->at(i) > 1){
...
//copied from MC cycle above
        if ( (PFDV_numberOfTracks_Out0p2->at(i) > 2 && ni_rho < 20 ) || (PFDV_numberOfTracks_0p2->at(i) > 2 && ni_rho >= 20 ) ){
          if ( PFDV_isNuclear->at(i)){
            //hPFDV_TrkV_R_isNuclearInteraction_Rebin_Barrel_3Tr0p2->Fill( ni_MC_rho );
	    ...
	    /*double PFDV_dtheta = fabs(PFDV_momentumInc_theta->at(i)-PFDV_momentumOut_theta->at(i));
            if (PFDV_dtheta > 3.14) PFDV_dtheta = fabs(2*3.14 - PFDV_dtheta); 
            if (PFDV_dtheta <= 0.5)hPFDV_momentum_dtheta-> Fill (PFDV_dtheta);
            else hPFDV_momentum_dtheta-> Fill (0.4999);*/
