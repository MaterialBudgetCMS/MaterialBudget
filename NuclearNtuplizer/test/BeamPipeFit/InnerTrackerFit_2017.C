...
// create function to fit the beam pipe with a tilted ellipse 
void funBeamPipeTiltedEllipse( Int_t &, Double_t *, Double_t &, Double_t *, Int_t );
...
////Average over 3 adjacent sectors to smooth differences 
  ...
  Double_t RangeEstimatorQuality, x_Sys = 0, r_Sys = 0, x0 = 0, y0 = 0, r0 = 1, r0_y =1, x0_Far = 0, y0_Far = 0, theta = 0. ;
  ...
  //FitObject = "BeamPipeTiltedEllipse"; 
  ...
  if(FitObject == "BeamPipeTiltedEllipse")
    {
    extraText = "work in progress";
    Rmin = 1.8, Rmax = 3.0, RBGmin = 2.35/*2.4*/, RBGmax = 3., RSmin = 2.0, RSmax = 2.35/*2.4*/, RPlot = 3.5;
    RangeEstimatorQuality = 0.1;
    ...
    y0 = -0.187; // from previous fits using this program that were based on 2017
    r0 = 2.211; // initial x radiu, from previous fits using this program that were based on 2015
    r0_y = 2.210; // in cm, initial y radius
    theta = 45.0; // degrees
    } // end if BeamPipeTiltedEllipse
  ...
  //for ( int k = 0; k < 1; k++ ) //for for list of histograms to fit
  {
    ...
    if ( FitObject == "PixelShield" || FitObject == "BeamPipe" || FitObject == "PixelShieldPlus" || FitObject == "PixelShieldMinus" ||
         FitObject == "BeamPipeEllipse" || FitObject == "BeamPipeTiltedEllipse"  || FitObject == "PixelShieldEllipse" ||
    ...
    if(FitObject == "BeamPipeTiltedEllipse")h->Rebin2D(5,5);
...
*/
    ...
    for ( UInt_t phiSect = 0; phiSect < 40; phiSect ++ )
    {
      ...
      { 
        ...
        if( (FitObject == "PixelSupport" || FitObject == "PixelSupportEllipse" || FitObject == "BeamPipe" || FitObject == "BeamPipeEllipse" || FitObject == "BeamPipeTiltedEllipse") 
...
/*     // Test the fit equation and how it is drawn
       fitterDraw->SetParameter( 0, "r0_x", 1.0, 0.01, 0.0, 3.5 ); // in cm
       fitterDraw->SetParameter( 1, "x0", 1.0, 0.01, -3.5,  3.5 ); // in cm
       fitterDraw->SetParameter( 2, "y0", -0.5, 0.01, -3.5, 3.5 ); // in cm
       fitterDraw->SetParameter( 3, "r0_y", 2.0, 0.01, 0.0, 3.5 ); // in cm
*/
       fitterDraw->FixParameter(0);
       fitterDraw->FixParameter(1);
       fitterDraw->FixParameter(2); 
    ...
    TGraph* gr_ellipseTilted = gr_arc;
    fitterTiltedEllipse = TVirtualFitter::Fitter(0,5);
    if ( FitObject == "BeamPipeTiltedEllipse")
      {
      //fitterTiltedEllipse = TVirtualFitter::Fitter(0,5);
      fitterTiltedEllipse->SetFCN(funBeamPipeTiltedEllipse);

      fitterTiltedEllipse->SetParameter(0, "r0_x", r0, 0.01, RSmin, RSmax );
      fitterTiltedEllipse->SetParameter( 1, "x0", x0, 0.01, -0.1,  0.2 );
      fitterTiltedEllipse->SetParameter( 2, "y0", y0, 0.01, -0.3,  0. );
      fitterTiltedEllipse->SetParameter( 3, "r0_y", r0_y, 0.01, RSmin, RSmax );
      fitterTiltedEllipse->SetParameter( 4, "theta", theta, 0.01, 0.0, 360);
/*
      fitterTiltedEllipse->SetParameter( 0, "r0_x", 2.0, 0.01, -3.5, 3.5 );
      fitterTiltedEllipse->SetParameter( 1, "x0", 1.0, 0.01, -3.5,  3.5 );
      fitterTiltedEllipse->SetParameter( 2, "y0", 0.0, 0.01, -3.5,  3.5 );
      fitterTiltedEllipse->SetParameter( 3, "r0_y", 1.5, 0.01, -3.5, 3.5 );
      fitterTiltedEllipse->SetParameter( 4, "theta", 45, 0.01, 0.0, 360.);
*/
      fitterTiltedEllipse->FixParameter(0);
      fitterTiltedEllipse->FixParameter(1);
      fitterTiltedEllipse->FixParameter(2);
      fitterTiltedEllipse->FixParameter(3);
      fitterTiltedEllipse->FixParameter(4);
 
      Double_t arglistArcEllipse[10] = {0.};
      fitterTiltedEllipse->ExecuteCommand( "MIGRAD", arglistArcEllipse, 0 );

      TEllipse* tiltedEllipse = new TEllipse( fitterTiltedEllipse->GetParameter(1), fitterTiltedEllipse->GetParameter(2), fitterTiltedEllipse->GetParameter(0), fitterTiltedEllipse->GetParameter(3),0,360,fitterTiltedEllipse->GetParameter(4));
      tiltedEllipse->SetFillStyle(0);
      tiltedEllipse->SetLineColor(kRed);
      tiltedEllipse->SetLineWidth(2);
      tiltedEllipse->Draw("same");
      Double_t x_tiltedEllipse[1], y_tiltedEllipse[2];
      x_tiltedEllipse[0] = fitterTiltedEllipse->GetParameter(1);
      y_tiltedEllipse[0] = fitterTiltedEllipse->GetParameter(2);

      gr_ellipseTilted = new TGraph(1, x_tiltedEllipse, y_tiltedEllipse);
      gr_ellipseTilted->SetMarkerStyle(22);
      gr_ellipseTilted->SetMarkerSize(1.5);
      gr_ellipseTilted->SetMarkerColor(kRed);
      gr_ellipseTilted->Draw("P");

    //  } // end tilted ellipse fit procedure

    // Draw Stats Box form tilted ellipse fit
    if( FitObject == "BeamPipeTiltedEllipse")
      {
      std::ostringstream legRadius;
      if(fitterTiltedEllipse->GetParError(0) >= ErrPrecision)legRadius << "#splitline{R_{x} (cm) \t = \t" << fixed << setprecision(3) << fitterTiltedEllipse->GetParameter(0) << " #pm " << fitterTiltedEllipse->GetParError(0) << " #pm " << r_Sys << "}";
      else legRadius << "#splitline{R_{x} (cm) \t = \t" << fixed << setprecision(3) << fitterTiltedEllipse->GetParameter(0) << " #pm " << r_Sys << "}";
      if(fitterTiltedEllipse->GetParError(3) >= ErrPrecision)legRadius << "{R_{y} (cm) \t = \t" << fixed << setprecision(3) << fitterTiltedEllipse->GetParameter(3) << " #pm " << fitterTiltedEllipse->GetParError(3) << " #pm " << r_Sys << "}";
      else legRadius << "{R_{y} (cm) \t = \t" << fixed << setprecision(3) << fitterTiltedEllipse->GetParameter(3) << " #pm " << r_Sys << "}";

      std::ostringstream legCenter;
      if(fitterTiltedEllipse->GetParError(1) >= ErrPrecision)legCenter << "#splitline{x_{0} (mm) \t = \t" << fixed << setprecision(2) << fitterTiltedEllipse->GetParameter(1)*10 << " #pm " << fitterTiltedEllipse->GetParError(1)*10 << " #pm " << x_Sys*10 << "}";
      else legCenter << "#splitline{x_{0} (mm) \t = \t" << fixed << setprecision(2) <<  fitterTiltedEllipse->GetParameter(1)*10 <<" #pm " << x_Sys*10 << "}";
      if(fitterTiltedEllipse->GetParError(2) >= ErrPrecision)legCenter << "{y_{0} (mm) \t = \t" << fixed << setprecision(2) << fitterTiltedEllipse->GetParameter(2)*10 << " #pm " << fitterTiltedEllipse->GetParError(2)*10 << " #pm " << x_Sys*10 << "}";
      else legCenter << "{y_{0} (mm) \t = \t" << fixed << setprecision(2) << fitterTiltedEllipse->GetParameter(2)*10 << " #pm " << x_Sys*10 << "}";

      if(fitterTiltedEllipse->GetParError(4) >= ErrPrecision)legCenter << "theta (rad) \t = \t" << fixed << setprecision(2) << fitterTiltedEllipse->GetParameter(4) << " #pm " << fitterTiltedEllipse->GetParError(4) << " #pm " << x_Sys;
      else legCenter << "theta (rad) \t = \t" << fixed << setprecision(2) << fitterTiltedEllipse->GetParameter(4) << " #pm " << x_Sys;
      ...
      } // end stats box creation for tilted ellipse
    ...
    if(FitObject == "PixelSupportEllipse" || FitObject == "PixelShieldEllipse" || FitObject == "BeamPipeEllipse" || FitObject == "BeamPipeTiltedEllipse")
     {
    ...
    if(FitObject != "PixelShieldEllipse" && FitObject != "PixelShield2Ellipses" && FitObject != "PixelShield2Arcs" && FitObject != "PixelShieldMinus" && FitObject != "PixelSupportMinus" && FitObject != "PixelSupportEllipse" && FitObject != "BeamPipeEllipse" && FitObject != "BeamPipeTiltedEllipse")
      {
    ...
    if(FitObject == "PixelSupportEllipse" || FitObject == "BeamPipeEllipse" || FitObject == "PixelShieldEllipse" /*|| FitObject == "BeamPipeTiltedEllipse"*/)
      {
    ...
    if(FitObject != "PixelShieldEllipse" && FitObject != "PixelShield2Ellipses" && FitObject != "PixelShield2Arcs" && FitObject != "PixelShieldMinus" && FitObject != "PixelSupportMinus" && FitObject != "PixelSupportEllipse" && FitObject != "BeamPipeEllipse" && FitObject != "BeamPipeTiltedEllispe")
      {
    ...
    // Draw tilted elipse in rhophi
    TF1 *bpTiltEllipseAlt = new TF1("bpTiltedAlt", func_TiltedEllipseRhoPhi, -3.15,3.15,5);
    if(FitObject == "BeamPipeTiltedEllipse")
      {
      bpTiltEllipseAlt->SetParameter(0,fitterTiltedEllipse->GetParameter(0)); 
      bpTiltEllipseAlt->SetParameter(1,fitterTiltedEllipse->GetParameter(1)); 
      bpTiltEllipseAlt->SetParameter(2,fitterTiltedEllipse->GetParameter(2)); 
      bpTiltEllipseAlt->SetParameter(3,fitterTiltedEllipse->GetParameter(3)); 
      bpTiltEllipseAlt->SetParameter(4,fitterTiltedEllipse->GetParameter(4));

      bpTiltEllipseAlt->SetLineColor(kRed);
      bpTiltEllipseAlt->SetLineWidth(2);
      bpTiltEllipseAlt->Draw("same");
      CMS_lumi(cPlots,iPeriod,0);
      ...
      legArc_RhoPhi->SetTextColor(kBlack);
      legArc_RhoPhi->AddEntry(arc,"Data 2017","");
      legArc_RhoPhi->AddEntry(arc,"Tilted Ellipse fit","l");

      legArc_RhoPhi->Draw("same");
      cPlots->Update();
      cPlots->SaveAs(("Plots/"+FitObject+"_Fit_RhoPhi.png"));
      cPlots->SaveAs(("Plots/"+FitObject+"_Fit_RhoPhi.pdf")); 
      } // end if for tilted ellipse in rhophi
   ...
   if (flag_Sys == 1)
       ...
       if(FitObject == "PixelShieldEllipse" || FitObject == "PixelSupportEllipse" || FitObject == "BeamPipeEllipse" || FitObject == "BeamPipeTiltedEllipse")
         {
         // set parameters for the positive deviation
...
Double_t func_TiltedEllipseRhoPhi(Double_t *x, Double_t *par)
{
  Double_t x_t = (par[0]*cos(x[0])*cos(par[4])) - (par[3]*sin(x[0])*sin(par[4]));
  Double_t y_t = (par[0]*cos(x[0])*sin(par[4])) + (par[3]*sin(x[0])*cos(par[4]));
  Double_t value = sqrt( (x_t+par[1])*(x_t+par[1]) + (y_t+par[2])*(y_t+par[2]) );
  return value;
}
...
// crreate the function to fit the beam pipe with a tilted ellipse
void funBeamPipeTiltedEllipse( Int_t &, Double_t *, Double_t &f, Double_t *par, Int_t )
{
Double_t resoNI = 0.01;
Int_t numBinsX = h->GetNbinsX();
Int_t numBinsY = h->GetNbinsY();
...
for ( UInt_t ix = 1; ix <= UInt_t(numBinsX); ix++ )
   ...
   for ( UInt_t iy = 1; iy <= UInt_t(numBinsY); iy++)
      ...
      // create variables needed for the tilted ellipse
      Double_t x = h->GetXaxis()->GetBinCenter(ix); // x position
      Double_t y = h->GetYaxis()->GetBinCenter(iy); // y position
      Double_t r = TMath::Sqrt( (x - par[1])*(x - par[1]) + (y - par[2])*(y - par[2]) );

      //TEllipse *tiltedEllipseTrial;
      //TEllipse tiltedEllipseTrial = new TEllipse(par[1],par[2],par[0],par[3],0,360,par[4]);
      TEllipse* tiltedEllipseTrial = new TEllipse(par[1],par[2],par[0],par[3],0,360,par[4]);

      //Double_t a = par[0]; // x radius
      //Double_t b = par[3]; // y radius
      //Double_t theta = par[4]; // angle of tilt in radians

      // use variables and parameters to write equaiton for ellipse
      /*Double_t x_ellipse = ((x - par[1])*TMath::Cos(theta) + (y - par[2])*TMath::Sin(theta));
      Double_t y_ellipse = ((x - par[1])*TMath::Sin(theta) - (y - par[2])*TMath::Cos(theta));*/

      //Double_t cos_ellipse = (x-par[1])/r; // the cosine for the ellipse
      //Double_t sin_ellipse = (y-par[2])/r; // the sine for the ellipse

      //Double_t x_ellipse = a*cos_ellipse;
      //Double_t y_ellipse = b*sin_ellipse;

      //Double_t x_prime = x_ellipse*cos(theta) - y_ellipse*sin(theta);
      //Double_t y_prime = x_ellipse*sin(theta) + y_ellipse*cos(theta);

      //Double_t r_ellipse = TMath::Sqrt(((x_ellipse * x_ellipse)) + ((y_ellipse * y_ellipse)));
      //Double_t r_ellipse = TMath::Sqrt(((x_prime * x_prime)) + ((y_prime * y_prime)));
      Double_t r_ellipse = TMath::Sqrt(((par[0] * par[0])) + ((par[3] * par[3])));
