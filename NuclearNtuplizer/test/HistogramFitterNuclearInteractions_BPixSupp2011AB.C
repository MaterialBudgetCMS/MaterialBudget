...
void HistogramFitterNuclearInteractions_BPixSupp2011AB()
{
  //gROOT->SetBatch(1);
  gROOT->ForceStyle();
  ...
  for ( int k = -7; k < 5; k++ )
  {
    ...
    fitter->SetParameter( 0,  "R",     21.7, 0.01, 21, 22.5 );
    fitter->SetParameter( 1, "x0",  0.06, 0.01, -0.7, 0.7 );
    fitter->SetParameter( 2, "y0", -0.18, 0.01, -0.7, 0.7 );
  ...
  */
  ...
  gRfit->GetYaxis()->SetRangeUser(21.6, 22);
  ...
  pad2->cd();
  gx0fit->SetTitle("");
  gx0fit->GetYaxis()->SetRangeUser(-1.3, -0.3);
  ...
  pad3->cd();
  gy0fit->SetTitle("");
  gy0fit->GetYaxis()->SetRangeUser(-3.8, -2.8);
